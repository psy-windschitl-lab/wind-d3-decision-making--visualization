...
- `src/vis/layoutBuilderFactory.ts` — the shared factory behind most layouts (`wizard`,
  `vanilla`, `gp1`, `gp2`, `table`). Individual files like `src/vis/gp2.ts` just call
  `createBuilderLayout({...config})` with different options (`previewMode`, `kind`,
  `restrictPreviewUntilFinish`). Prefer adding config branches / a layout-name check here
  over duplicating this file — it's ~1000 lines, and duplicating it was explicitly called
  out as a past failure mode (partial copies, drift between copies).
...
- `src/lib/layoutTutorial.ts` — the "how to read your layout" walkthrough using a fixed
  ...
  callouts. **Currently GP2-only** — gated in `layoutBuilderFactory.ts` via
  `root.parentElement?.dataset.layout === "gp2"`. Don't let it leak into other layouts
  (wizard, GP1, etc.) unless explicitly asked to expand its scope.
- `src/pages/chooser.ts` — the "Choose a Version" picker; layout options are described
  here for end users.
- Query params driving behavior: `layout` (wizard/vanilla/gp1/gp2/manual/table),
